...
// API Présences
export const fetchStudentsByPromotion = async (promotion: string, token: string): Promise<Student[]> => {
  await new Promise(resolve => setTimeout(resolve, 400));
...
export const submitAttendance = async (attendanceRecord: Omit<AttendanceRecord, 'id'>, token: string): Promise<boolean> => {
  await new Promise(resolve => setTimeout(resolve, 800));
  return true;
};

export const fetchTeacherAttendance = async (teacherId: string, token: string): Promise<AttendanceRecord[]> => {
  await new Promise(resolve => setTimeout(resolve, 500));
  ...
  return [
    {
      ...
      promotion: 'L3 RT',
      students: [
        { studentId: '1', studentName: 'Djeukeng Kana', isPresent: true },
        { studentId: '5', studentName: 'Alice Mbarga', isPresent: false },
        { studentId: '6', studentName: 'Jean Fotso', isPresent: true, isLate: true }
      ],
      isSubmitted: true,
      submittedAt: '2024-01-29T08:15:00Z'
    }
  ];
};

export const fetchStudentAbsences = async (studentId: string, token: string): Promise<AbsenceRecord[]> => {
  await new Promise(resolve => setTimeout(resolve, 400));
